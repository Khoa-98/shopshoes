(function ($) {
    ...
    if ($('#flotDashSales1').get(0)) {
        var flotDashSales1 = $.plot('#flotDashSales1', flotDashSales1Data, {
            ...
            yaxis: {min: 0, color: 'rgba(0,0,0,0.1)'},
            xaxis: {mode: 'categories', color: 'rgba(0,0,0,0)'},
            legend: {show: false},
            tooltip: true,
            tooltipOpts: {content: '%x: %y', shifts: {x: -30, y: 25}, defaultTheme: false}
        });
    ...
    if ($('#meterSalesSel').get(0)) {
        $('#meterSalesSel a').on('click', function (ev) {
            ev.preventDefault();
            var val = $(this).data("val"), selector = $(this).parent(), items = selector.find('a');
    ...
    if ($('#flotDashBasic').get(0)) {
        var flotDashBasic = $.plot('#flotDashBasic', flotDashBasicData, {
            ...
            yaxis: {min: 0, max: 200, color: 'rgba(0,0,0,0.1)'},
            xaxis: {color: 'rgba(0,0,0,0)'},
            tooltip: true,
    ...
    if ($('#sparklineBarDash').get(0)) {
        ...
        $(this).on("styleSwitcher.modifyVars", function (ev) {
            $("#sparklineBarDash").sparkline(sparklineBarDashData, $.extend({}, sparklineBarDashOptions, {barColor: ev.options.colorPrimary}));
        });
        if (typeof ($.browser) != 'undefined') {
            if ($.browser.mobile) {
                $("#sparklineBarDash").sparkline(sparklineBarDashData, $.extend({}, sparklineBarDashOptions, {barColor: '#0088cc'}));
            }
        }
    }
    if ($('#sparklineLineDash').get(0)) {
        var sparklineLineDashOptions = {type: 'line', width: '80', height: '55', lineColor: '#CCCCCC'};
